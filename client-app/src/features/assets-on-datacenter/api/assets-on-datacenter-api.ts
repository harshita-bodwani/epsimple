...
const ASSETS_ON_Datacenter_ENDPOINTS = {
  ...
  BY_Datacenter: (DatacenterId: number) =>
    `/api/assets-on-datacenter/datacenter/${DatacenterId}`,
...
export const assetsOnDatacenterApi = {
    ...
    return useQuery({
      ...
      queryFn: async () => {
        const response = await api.get<
          ApiResponse<BackendPageResponse<AssetsOnDatacenter>>
  ...
  useCreate: () => {
    ...
    return useMutation({
      ...
      onError: (error: any) => {
        toast.error(
          error.response?.data?.message || 'Failed to place asset in Datacenter'
        )
      },
    })
  },
  ...
  useUpdate: () => {
    ...
    return useMutation({
      mutationFn: async ({
        ...
        id: number
        data: AssetsOnDatacenterFormData
      }) => {
      ...
      onError: (error: any) => {
        toast.error(
          error.response?.data?.message ||
            'Failed to update asset in Datacenter'
        )
      },
    })
  },
  ...
  useDelete: () => {
    ...
    return useMutation({
      ...
      onError: (error: any) => {
        toast.error(
          error.response?.data?.message ||
            'Failed to remove asset from Datacenter'
        )
      },
    })
  },
