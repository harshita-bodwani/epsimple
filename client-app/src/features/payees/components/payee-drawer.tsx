...
export function PayeeDrawer() {
  ...
  const { data: payeeDetailsData, isLoading: isPayeeDetailsLoading } =
    useSearchPayeeDetails({
  ...
  const { data: landlords = [], isLoading: isLandlordsLoading } =
    useSearchLandlords(landlordSearch)
  ...
  const displayPayeeTypes = (() => {
    ...
    const selected = allPayeeTypes.find(
      (t) => t.id === selectedPayee.payeeTypeId
    )
  ...
  const displayVendors = (() => {
    ...
    const selected = allVendors.find(
      (v: Vendor) => v.id === selectedPayee.vendorId
    )
  ...
  const displayLandlords = (() => {
    ...
    const selected = allLandlords.find(
      (l: Landlord) => l.id === selectedPayee.landlordId
    )
    ...
    <Sheet open={isDrawerOpen} onOpenChange={closeDrawer}>
      <SheetContent className='flex flex-col sm:max-w-[500px]'>
        <SheetHeader className='text-start'>
          <SheetTitle>{selectedPayee ? 'Update' : 'Create'} Payee</SheetTitle>
          <SheetDescription>
            {selectedPayee ? 'Update the payee details.' : 'Add a new payee.'}
                  ...
                  <Popover open={payeeTypeOpen} onOpenChange={setPayeeTypeOpen}>
                    <PopoverTrigger asChild>
                      <FormControl>
                        ...
                        >
                          {field.value
                            ? displayPayeeTypes.find(
                                (t) => t.id === field.value
                              )?.payeeType || 'Select payee type'
                            : 'Select payee type'}
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem className='flex flex-col'>
                  ...
                  <Popover
                    open={payeeDetailsOpen}
                    onOpenChange={setPayeeDetailsOpen}
                  >
                    <PopoverTrigger asChild>
                      <FormControl>
                        <Button
                  ...
                  <Popover open={landlordOpen} onOpenChange={setLandlordOpen}>
                    <PopoverTrigger asChild>
                      <FormControl>
                        ...
                        >
                          {field.value
                            ? displayLandlords.find(
                                (l: Landlord) => l.id === field.value
                              )?.landlordName || 'Select landlord'
                            : 'None'}
                          <ChevronsUpDown className='ml-2 h-4 w-4 shrink-0 opacity-50' />
                        </Button>
                      </FormControl>
        ...
        <SheetFooter className='flex-shrink-0 px-4'>
          ...
          <Button type='submit' form='payee-form' disabled={isLoading}>
