...
export const costCategoriesApi = {
  ...
  getAll: async (
    ...
    const response = await api.get<
      ApiResponse<BackendPageResponse<CostCategory>>
    >(BASE_URL, {
  ...
  getList: async (): Promise<ApiResponse<CostCategory[]>> => {
    const response = await api.get<ApiResponse<CostCategory[]>>(
      `${BASE_URL}/list`
    )
    return response.data
  },
  ...
  update: async (
    ...
    const response = await api.put<ApiResponse<CostCategory>>(
      `${BASE_URL}/${id}`,
      data
    )
    return response.data
...
export const getAllCostCategoriesList = async (): Promise<
  ApiResponse<CostCategory[]>
> => {
