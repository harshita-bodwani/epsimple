...
const vendorsApi = {
  getAll: async (
  ...
  ): Promise<FlatPageResponse<Vendor>> => {
    const {
      page = 0,
      size = 10,
      sortBy = 'id',
    ...
    if (search && search.trim()) {
      const response = await api.get<ApiResponse<BackendPageResponse<Vendor>>>(
        '/api/vendors/search',
        {
