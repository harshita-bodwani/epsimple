...
import {
  assetLocationApi,
  type AssetLocationCheck,
} from '../api/asset-location-api'
...
export function AssetPlacementDialog({
  ...
  const [existingLocation, setExistingLocation] =
    useState<AssetLocationCheck | null>(null)
  ...
  const placementMutation = useMutation({
    mutationFn: async (data: PlacementFormData) => {
      const endpoint =
        data.locationType === LOCATION_TYPES.SITE
          ? '/api/assets-on-site'
          : data.locationType === LOCATION_TYPES.WAREHOUSE
            ? '/api/assets-on-warehouse'
            : '/api/assets-on-datacenter'

      const payload = {
        assetId,
        ...(data.locationType === LOCATION_TYPES.SITE && {
          siteId: data.locationId,
        }),
        ...(data.locationType === LOCATION_TYPES.WAREHOUSE && {
          warehouseId: data.locationId,
        }),
        ...(data.locationType === LOCATION_TYPES.DATACENTER && {
          datacenterId: data.locationId,
        }),
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({
        queryKey: ['asset-movement-history', assetId],
      })
        ...
        <DialogContent className='max-h-[90vh] max-w-2xl overflow-y-auto'>
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className='space-y-4'>
                  ...
                  <FormItem>
                    ...
                    <Select
                      ...
                      onValueChange={(value) => {
                        ...
                        setLocationType(
                          value as (typeof LOCATION_TYPES)[keyof typeof LOCATION_TYPES]
                        )
                    ...
                    >
                      ...
                      <SelectContent>
                        <SelectItem value={LOCATION_TYPES.SITE}>
                          Site
                        </SelectItem>
                        <SelectItem value={LOCATION_TYPES.WAREHOUSE}>
                          Warehouse
                        </SelectItem>
                        <SelectItem value={LOCATION_TYPES.DATACENTER}>
                          Datacenter
                        </SelectItem>
                      </SelectContent>
                    </Select>
                            ...
                            {field.value
                              ? (() => {
                                  ...
                                  if (locationType === LOCATION_TYPES.SITE) {
                                    return (location as { siteCode: string })
                                      .siteCode
                                  } else if (
                                    locationType === LOCATION_TYPES.WAREHOUSE
                                  ) {
                                    return (
                                      location as { warehouseName: string }
                                    ).warehouseName
                                  } else {
                                    return (
                                      location as { datacenterName: string }
                                    ).datacenterName
                                  }
                                })()
                              : 'Select location'}
                    ...
                    >
                      <PopoverTrigger asChild>
                        <FormControl>
                          ...
                          >
                            {field.value
                              ? activityWorks.find((w) => w.id === field.value)
                                  ?.vendorOrderNumber
              ...
              {(locationType === LOCATION_TYPES.WAREHOUSE ||
                locationType === LOCATION_TYPES.DATACENTER) && (
                <>
                  <FormField
                    control={form.control}
                    name='commissionedOn'
