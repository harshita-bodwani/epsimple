...
const AssetTagCodeGeneratorContext = createContext<
  AssetTagCodeGeneratorContextType | undefined
>(undefined)

export function AssetTagCodeGeneratorProvider({
  children,
}: {
  children: ReactNode
}) {
  ...
  const [editingGenerator, setEditingGenerator] =
    useState<AssetTagCodeGenerator | null>(null)
...
export function useAssetTagCodeGeneratorContext() {
  ...
  if (!context) {
    throw new Error(
      'useAssetTagCodeGeneratorContext must be used within a AssetTagCodeGeneratorProvider'
    )
  }
  return context
}
