...
import type {
  AssetTagCodeGenerator,
  AssetTagCodeGeneratorFormData,
  GeneratedAssetTag,
} from './schema'
...
export const assetTagCodeGeneratorApi = {
    ...
    return useQuery({
      ...
      queryFn: async () => {
        const endpoint = params.search
          ? ASSET_TAG_GENERATOR_ENDPOINTS.SEARCH
          : ASSET_TAG_GENERATOR_ENDPOINTS.BASE
        const response = await api.get<
          ApiResponse<BackendPageResponse<AssetTagCodeGenerator>>
        >(endpoint, {
          params: {
            page: params.page,
            size: params.size,
    ...
    return useMutation({
      ...
      onError: (error: any) => {
        const errorMessage =
          ...
          'Failed to create asset tag generator'
  ...
  useUpdate: () => {
    ...
    return useMutation({
      mutationFn: async ({
        ...
        id: number
        data: AssetTagCodeGeneratorFormData
      }) => {
      ...
      onError: (error: any) => {
        const errorMessage =
          error?.response?.data?.message ||
          'Failed to update asset tag generator'
        toast.error(errorMessage)
      },
    })
  },
  ...
  useDelete: () => {
    ...
    return useMutation({
      ...
      onError: (error: any) => {
        const errorMessage =
          error?.response?.data?.message ||
          'Failed to delete asset tag generator'
        toast.error(errorMessage)
      },
    })
  },
  ...
  useGenerateTag: () => {
    ...
    return useMutation({
      ...
      onError: (error: any) => {
        const errorMessage =
          error?.response?.data?.message || 'Failed to generate asset tag'
        toast.error(errorMessage)
      },
    })
  },
  ...
  usePreviewTag: () => {
    return useMutation({
      ...
      onError: (error: any) => {
        const errorMessage =
          error?.response?.data?.message || 'Failed to preview asset tag'
        toast.error(errorMessage)
      },
    })
  },
  ...
  getAll: async (params: {
    ...
    const endpoint = params.searchTerm
      ? ASSET_TAG_GENERATOR_ENDPOINTS.SEARCH
      : ASSET_TAG_GENERATOR_ENDPOINTS.BASE
    const response = await api.get<
      ApiResponse<BackendPageResponse<AssetTagCodeGenerator>>
  ...
  getList: async (): Promise<AssetTagCodeGenerator[]> => {
    const response = await api.get<ApiResponse<AssetTagCodeGenerator[]>>(
      ASSET_TAG_GENERATOR_ENDPOINTS.LIST
    )
    return response.data.data
  },
}
