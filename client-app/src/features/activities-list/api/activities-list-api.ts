...
export const activitiesListApi = {
  create: async (
    data: ActivitiesListFormData
  ): Promise<ApiResponse<ActivitiesList>> => {
  ...
  getAll: async (
    ...
    const response = await api.get<
      ApiResponse<BackendPageResponse<ActivitiesList>>
    >(BASE_URL, {
  ...
  getList: async (): Promise<ApiResponse<ActivitiesList[]>> => {
    const response = await api.get<ApiResponse<ActivitiesList[]>>(
      `${BASE_URL}/list`
    )
    return response.data
  },
  ...
  update: async (
    ...
    const response = await api.put<ApiResponse<ActivitiesList>>(
      `${BASE_URL}/${id}`,
      data
    )
    return response.data
