...
export const siteCodeGeneratorApi = {
    ...
    return useQuery({
      ...
      queryFn: async () => {
        const endpoint = params.search
          ? SITE_CODE_GENERATOR_ENDPOINTS.SEARCH
          : SITE_CODE_GENERATOR_ENDPOINTS.BASE
        const response = await api.get<
          ApiResponse<BackendPageResponse<SiteCodeGenerator>>
        >(endpoint, {
          params: {
            page: params.page,
            size: params.size,
    ...
    return useMutation({
      ...
      onError: (error: any) => {
        const errorMessage =
          ...
          'Failed to create site code generator'
  ...
  useUpdate: () => {
    ...
    return useMutation({
      mutationFn: async ({
        ...
        id: number
        data: SiteCodeGeneratorFormData
      }) => {
      ...
      onError: (error: any) => {
        const errorMessage =
          error?.response?.data?.message ||
          'Failed to update site code generator'
        toast.error(errorMessage)
      },
    })
  },
  ...
  useDelete: () => {
    ...
    return useMutation({
      ...
      onError: (error: any) => {
        const errorMessage =
          error?.response?.data?.message ||
          'Failed to delete site code generator'
        toast.error(errorMessage)
      },
    })
  },
  ...
  useGenerateCode: () => {
    ...
    return useMutation({
      ...
      onError: (error: any) => {
        const errorMessage =
          error?.response?.data?.message || 'Failed to generate site code'
        toast.error(errorMessage)
      },
    })
  },
  ...
  usePreviewCode: () => {
    return useMutation({
      ...
      onError: (error: any) => {
        const errorMessage =
          error?.response?.data?.message || 'Failed to preview site code'
        toast.error(errorMessage)
      },
    })
  },
  ...
  getList: async (): Promise<SiteCodeGenerator[]> => {
    const response = await api.get<ApiResponse<SiteCodeGenerator[]>>(
      SITE_CODE_GENERATOR_ENDPOINTS.LIST
    )
    return response.data.data
  },
}
