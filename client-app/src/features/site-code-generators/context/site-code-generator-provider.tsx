...
const SiteCodeGeneratorContext = createContext<
  SiteCodeGeneratorContextType | undefined
>(undefined)

export function SiteCodeGeneratorProvider({
  children,
}: {
  children: ReactNode
}) {
  ...
  const [editingGenerator, setEditingGenerator] =
    useState<SiteCodeGenerator | null>(null)
...
export function useSiteCodeGeneratorContext() {
  ...
  if (!context) {
    throw new Error(
      'useSiteCodeGeneratorContext must be used within a SiteCodeGeneratorProvider'
    )
  }
  return context
}
