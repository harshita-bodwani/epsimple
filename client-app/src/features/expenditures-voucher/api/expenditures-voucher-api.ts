...
export const expendituresVoucherApi = {
  create: async (
    data: ExpendituresVoucherFormData
  ): Promise<ApiResponse<ExpendituresVoucher>> => {
    const response = await api.post<ApiResponse<ExpendituresVoucher>>(
      BASE_URL,
      data
    )
  ...
  getAll: async (
    ...
    const response = await api.get<
      ApiResponse<BackendPageResponse<ExpendituresVoucher>>
    >(BASE_URL, {
  ...
  update: async (
    ...
    const response = await api.put<ApiResponse<ExpendituresVoucher>>(
      `${BASE_URL}/${id}`,
      data
    )
    return response.data
  ...
  useCreate: () => {
    ...
    return useMutation({
      ...
      onError: (error: any) => {
        const errorMessage =
          error?.response?.data?.message || 'Failed to create expenditure'
  ...
  useUpdate: () => {
    ...
    return useMutation({
      ...
      onError: (error: any) => {
        const errorMessage =
          error?.response?.data?.message || 'Failed to update expenditure'
        toast.error(errorMessage)
      },
    })
  },
