...
export function BasicTab({ form, states }: Readonly<BasicTabProps>) {
  ...
  const { data: personDetails = [], isLoading: isLoadingPersons } =
    personDetailsApi.useSearch(personSearch)
  ...
  const handleGenerateCode = async () => {
    ...
    const stateId = states.find(
      (state) => state.stateName === selectedLocation?.stateName
    )?.id
    if (!stateId) {
        ...
        <FormField
          ...
          render={({ field }) => {
            const selectedProject = field.value
              ? displayProjects.find((p) => p.id === field.value)
              : null
              ...
              <Popover open={categoryOpen} onOpenChange={setCategoryOpen}>
                <PopoverTrigger asChild>
                  <FormControl>
                    ...
                    >
                      {field.value
                        ? displayCategories.find((c) => c.id === field.value)
              ...
              <Popover open={typeOpen} onOpenChange={setTypeOpen}>
                <PopoverTrigger asChild>
                  <FormControl>
                    ...
                    >
                      {field.value
                        ? displayTypes.find((t) => t.id === field.value)
              ...
              <Popover open={statusOpen} onOpenChange={setStatusOpen}>
                <PopoverTrigger asChild>
                  <FormControl>
                    ...
                    >
                      {field.value
                        ? statuses.find((s) => s.id === field.value)
                      ...
                      >
                        {field.value
                          ? personDetails.find((p) => p.id === field.value)
                            ? personDetails.find((p) => p.id === field.value)
                                ?.fullName ||
                              `${personDetails.find((p) => p.id === field.value)?.firstName || ''} ${personDetails.find((p) => p.id === field.value)?.lastName || ''}`.trim() ||
                              'Unknown'
                            : 'Select contact'
                          : 'Select contact'}
                        <ChevronsUpDown className='ml-2 h-4 w-4 shrink-0 opacity-50' />
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem className='flex-1'>
                ...
                <Popover
                  open={regionalManagerOpen}
                  onOpenChange={setRegionalManagerOpen}
                >
                  <PopoverTrigger asChild>
                    <FormControl>
                      <Button
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem className='flex-1'>
                ...
                <Popover
                  open={masterFranchiseeOpen}
                  onOpenChange={setMasterFranchiseeOpen}
                >
                  <PopoverTrigger asChild>
                    <FormControl>
                      <Button
