...
export const siteTypeApi = {
  create: async (data: SiteTypeFormData): Promise<SiteType> => {
    const response = await api.post<ApiResponse<SiteType>>(
      SITE_TYPE_ENDPOINTS.BASE,
      data
    )
  ...
  getAll: async (params: {
    ...
    const response = await api.get<ApiResponse<BackendPageResponse<SiteType>>>(
      SITE_TYPE_ENDPOINTS.BASE,
      { params }
    )
  ...
  getList: async (): Promise<SiteType[]> => {
    const response = await api.get<ApiResponse<SiteType[]>>(
      SITE_TYPE_ENDPOINTS.LIST
    )
    return response.data.data // Unwrap ApiResponse
  },
  ...
  getById: async (id: number): Promise<SiteType> => {
    const response = await api.get<ApiResponse<SiteType>>(
      SITE_TYPE_ENDPOINTS.BY_ID(id)
    )
    return response.data.data // Unwrap ApiResponse
  },

  update: async (id: number, data: SiteTypeFormData): Promise<SiteType> => {
    const response = await api.put<ApiResponse<SiteType>>(
      SITE_TYPE_ENDPOINTS.BY_ID(id),
      data
    )
    return response.data.data // Unwrap ApiResponse
    ...
    return useQuery({
      ...
      queryFn: async () => {
        ...
        const response = await api.get<
          ApiResponse<BackendPageResponse<SiteType>>
        >(endpoint, { params })
