...
const landlordsApi = {
  getAll: async (
    params: GetAllLandlordsParams = {}
  ): Promise<FlatPageResponse<Landlord>> => {
    const {
      page = 0,
      size = 10,
      sortBy = 'id',
    ...
    if (search?.trim()) {
      const response = await api.get<
        ApiResponse<BackendPageResponse<Landlord>>
    ...
    const response = await api.get<ApiResponse<BackendPageResponse<Landlord>>>(
      '/api/landlords',
      {
  ...
  getList: async (): Promise<Landlord[]> => {
    const response = await api.get<ApiResponse<Landlord[]>>(
      '/api/landlords/list'
    )
  ...
  create: async (data: LandlordFormData): Promise<Landlord> => {
    const response = await api.post<ApiResponse<Landlord>>(
      '/api/landlords',
      data
    )
    return response.data.data
  ...
  update: async (id: number, data: LandlordFormData): Promise<Landlord> => {
    const response = await api.put<ApiResponse<Landlord>>(
      `/api/landlords/${id}`,
      data
    )
    return response.data.data
