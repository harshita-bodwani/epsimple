...
const PERSON_DETAILS_ENDPOINTS = {
  ...
  BY_PERSON_TYPE: (personTypeId: number) =>
    `/api/person-details/person-type/${personTypeId}`,
    ...
    return useQuery({
      ...
      queryFn: async () => {
        const response = await api.get<
          ApiResponse<BackendPageResponse<PersonDetails>>
    ...
    return useQuery({
      ...
      queryFn: async () => {
        const endpoint = searchTerm?.trim()
          ? PERSON_DETAILS_ENDPOINTS.SEARCH
          : PERSON_DETAILS_ENDPOINTS.BASE
  ...
  useCreate: () => {
    ...
    return useMutation({
      mutationFn: async (data: PersonDetailsFormData) => {
        const response = await api.post<ApiResponse<PersonDetails>>(
          PERSON_DETAILS_ENDPOINTS.BASE,
          data
        )
      ...
      onError: (error: any) => {
        const errorMessage =
          error?.response?.data?.message || 'Failed to create person details'
  ...
  useUpdate: () => {
    ...
    return useMutation({
      mutationFn: async ({
        ...
        id: number
        data: PersonDetailsFormData
      }) => {
        const response = await api.put<ApiResponse<PersonDetails>>(
          PERSON_DETAILS_ENDPOINTS.BY_ID(id),
          data
        )
        return response.data.data
      },
      ...
      onError: (error: any) => {
        const errorMessage =
          error?.response?.data?.message || 'Failed to update person details'
        toast.error(errorMessage)
      },
    })
  },
  ...
  useDelete: () => {
    ...
    return useMutation({
      ...
      onError: (error: any) => {
        ...
        const errorMessage =
          error?.response?.data?.message || 'Failed to delete person details'
        toast.error(errorMessage)
      },
    })
  },
