...
export function CostTypeDrawer() {
  ...
  const displayCostCategories = (() => {
    ...
    const selectedCategory = allCostCategories.find(
      (c) => c.id === selectedCostType.costCategoryId
    )
    if (!selectedCategory) return costCategories
    // Check if selected category is already in the list
    if (costCategories.some((c) => c.id === selectedCategory.id))
    ...
    <Sheet open={isDrawerOpen} onOpenChange={handleClose}>
      <SheetContent className='flex flex-col'>
        <SheetHeader className='text-start'>
          <SheetTitle>{isEditMode ? 'Update' : 'Create'} Cost Type</SheetTitle>
          <SheetDescription>
            {isEditMode
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem className='flex flex-col'>
                  ...
                  <Popover
                    open={costCategoryOpen}
                    onOpenChange={setCostCategoryOpen}
                  >
        ...
        <SheetFooter className='gap-2 pt-4'>
          ...
          <Button type='submit' form='cost-type-form' disabled={isLoading}>
