...
export const siteCategoryApi = {
    ...
    return useQuery({
      ...
      queryFn: async () => {
        const response = await api.get<
          ApiResponse<BackendPageResponse<SiteCategory>>
  ...
  useCreate: () => {
    ...
    return useMutation({
      ...
      onError: (error: any) => {
        const errorMessage =
          error?.response?.data?.message || 'Failed to create site category'
  ...
  useUpdate: () => {
    ...
    return useMutation({
      ...
      onError: (error: any) => {
        const errorMessage =
          error?.response?.data?.message || 'Failed to update site category'
        toast.error(errorMessage)
      },
    })
  },
  ...
  useDelete: () => {
    ...
    return useMutation({
      ...
      onError: (error: any) => {
        const errorMessage =
          error?.response?.data?.message || 'Failed to delete site category'
        toast.error(errorMessage)
      },
    })
  },
  ...
  getList: async (): Promise<SiteCategory[]> => {
    const response = await api.get<ApiResponse<SiteCategory[]>>(
      SITE_CATEGORY_ENDPOINTS.LIST
    )
    return response.data.data // Unwrap ApiResponse
  },
