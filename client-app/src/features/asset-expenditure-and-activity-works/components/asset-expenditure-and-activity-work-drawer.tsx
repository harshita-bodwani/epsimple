...
import {
  assetExpenditureAndActivityWorkFormSchema,
  type AssetExpenditureAndActivityWorkFormData,
} from '../api/schema'
...
export function AssetExpenditureAndActivityWorkDrawer() {
  const { isDrawerOpen, closeDrawer, selectedExpenditure, assetId } =
  ...
  const currentAsset = assetId
    ? allAssets.find(
        (a: { id: number; assetTagId: string }) => a.id === assetId
      )
    : null
  ...
  useEffect(() => {
    if (selectedExpenditure) {
      form.reset({
        ...
        expendituresInvoiceId:
          selectedExpenditure.expendituresInvoiceId || undefined,
      })
    } else {
      form.reset({
        assetId: assetId || 0,
  ...
  const onSubmit = async (data: AssetExpenditureAndActivityWorkFormData) => {
    if (selectedExpenditure) {
      updateMutation.mutate(
        { id: selectedExpenditure.id, data },
        {
          onSuccess: () => {
            closeDrawer()
            form.reset()
  ...
  return (
    <Sheet open={isDrawerOpen} onOpenChange={(open) => !open && closeDrawer()}>
      <SheetContent className='w-full overflow-y-auto sm:max-w-xl'>
        <SheetHeader>
          <SheetTitle>
            {selectedExpenditure ? 'Edit' : 'Create'} Asset Expenditure
          </SheetTitle>
          <SheetDescription>
            {selectedExpenditure ? 'Update' : 'Create new'} asset expenditure
            and activity work linkage
          </SheetDescription>
        </SheetHeader>
                  ...
                  >
                    <PopoverTrigger asChild>
                      <FormControl>
                        ...
                        >
                          {field.value
                            ? activityWorks.find((aw) => aw.id === field.value)
                                ?.vendorOrderNumber ||
                              activityWorks.find((aw) => aw.id === field.value)
                                ?.activitiesName ||
