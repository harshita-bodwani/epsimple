...
import type {
  AssetExpenditureAndActivityWorkFormData,
  AssetExpenditureAndActivityWork,
} from './schema'
...
const EXPENDITURE_ENDPOINTS = {
  ...
  BY_ASSET: (assetId: number) =>
    `/api/asset-expenditure-and-activity-works/asset/${assetId}`,
  BY_ACTIVITY_WORK: (activityWorkId: number) =>
    `/api/asset-expenditure-and-activity-works/activity-work/${activityWorkId}`,
...
export const assetExpenditureAndActivityWorkApi = {
    ...
    return useQuery({
      ...
      queryFn: async () => {
        const response = await api.get<
          ApiResponse<BackendPageResponse<AssetExpenditureAndActivityWork>>
  ...
  useGetByAssetId: (
    ...
    return useQuery({
      queryKey: [
        'asset-expenditure-and-activity-works',
        'asset',
        assetId,
        params,
      ],
  ...
  useGetById: (id: number) => {
    return useQuery({
      ...
      queryFn: async () => {
        const response = await api.get<
          ApiResponse<AssetExpenditureAndActivityWork>
    ...
    return useMutation({
      mutationFn: async (data: AssetExpenditureAndActivityWorkFormData) => {
        const response = await api.post<
          ApiResponse<AssetExpenditureAndActivityWork>
        >(EXPENDITURE_ENDPOINTS.BASE, data)
        return response.data
      },
      onSuccess: () => {
        queryClient.invalidateQueries({
          queryKey: ['asset-expenditure-and-activity-works'],
        })
  ...
  useUpdate: () => {
    ...
    return useMutation({
      mutationFn: async ({
        ...
        id: number
        data: AssetExpenditureAndActivityWorkFormData
      }) => {
        const response = await api.put<
          ApiResponse<AssetExpenditureAndActivityWork>
        >(EXPENDITURE_ENDPOINTS.BY_ID(id), data)
        return response.data
      },
      onSuccess: () => {
        queryClient.invalidateQueries({
