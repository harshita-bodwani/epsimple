...
const ASSETS_ON_WAREHOUSE_ENDPOINTS = {
  ...
  BY_WAREHOUSE: (warehouseId: number) =>
    `/api/assets-on-warehouse/warehouse/${warehouseId}`,
...
export const assetsOnWarehouseApi = {
    ...
    return useQuery({
      ...
      queryFn: async () => {
        const response = await api.get<
          ApiResponse<BackendPageResponse<AssetsOnWarehouse>>
  ...
  useCreate: () => {
    ...
    return useMutation({
      ...
      onError: (error: any) => {
        toast.error(
          error.response?.data?.message || 'Failed to place asset in warehouse'
        )
      },
    })
  },
  ...
  useUpdate: () => {
    ...
    return useMutation({
      mutationFn: async ({
        ...
        id: number
        data: AssetsOnWarehouseFormData
      }) => {
      ...
      onError: (error: any) => {
        toast.error(
          error.response?.data?.message || 'Failed to update asset in warehouse'
        )
      },
    })
  },
      ...
      onError: (error: any) => {
        toast.error(
          error.response?.data?.message ||
            'Failed to remove asset from warehouse'
        )
      },
    })
  },
